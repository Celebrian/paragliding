...
Here I used a .env file with these environment variables and tested with "go install && heroku local"
...
```
For the track IDs in the mongo database I choose to use the mongo autogenerated ID, even if there could arrise a problem later, as people can extract the upload date from the ID. 
In this case there is no sensitive information in the track as a whole, and one of the alternatives would be a sequential counter to increment the ID, but that would be vulnerable to en enumeration attack, witch in this case would be a bigger issue in my opinion. 
The safest way would be to generate a random string and check if it is in use, use it if it was not, and generate a new one if it was. I did not have time to do that on this assignment but it would be on a roadmap if this application was going to be developed further.
